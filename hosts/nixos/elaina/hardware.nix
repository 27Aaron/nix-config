...
}:
{
  ...
  # This platform only supports s2idle (the BIOS exposes no S3). Closing the
  # lid suspends first and hands over to hibernate after an hour. On AC power,
  # the countdown does not start, so the machine stays suspended. When docked,
  # closing the lid is ignored so the internal panel remains usable with an
